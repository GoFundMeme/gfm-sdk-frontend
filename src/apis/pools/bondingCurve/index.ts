...
import {
  createBondingCurveProcess,
  createBondingCurveRequest,
} from "./createBondingCurvePool";
import type { Program } from "@coral-xyz/anchor";
import { Gofundmeme } from "../../../IDL";

export const buildBondingCurveApiUtils = async (
  poolUtils: PoolsUtils,
  gfmProgram: Program<Gofundmeme>
) => {
  const processCreatePool = async (
    payload: CreateBondingCurveProcessPayload
  ) => {
    ...
    return {
      ...
      signAndConfirm: async ({ creator }: { creator: Keypair }) => {
        if (
          ...
          payload.creatorWalletAddress.toString()
        ) {
          throw new Error(
            `Incorrect signer (${creator.publicKey.toString()}). Should be ${payload.creatorWalletAddress.toString()} `
          );
        }
